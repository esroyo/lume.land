...
- `basename`: Define the page's basename.
...
The `date` variable is defined using
[the `date` variable](../creating-pages/page-data.md#date). Lume will try to
...
For example, the following markdown file is saved in the `posts/hello-world.md`
file:
...
```js
{
  ...
  date: new Date(), // file creation date,
...
While `Page.data` returns the `Data` object of the page, `Page.text` returns the
result of rendering the data or, in other words, the content that will be output
to the dest folder as a string. You can use processors to modify this content:
...
unlisted `boolean`
: Pages that must not listed in the [sitemap](../../plugins/sitemap.md) or similar. It's generated by [redirects](../../plugins/redirects.md) or [decap_cms](../../plugins/decap_cms.md) plugins.
