...
If the file `styles.css` **doesn't exist locally**, Lume use the content of the
remote URL as fallback. Note that the file **won't be saved in the project
...
Some specifiers that are compatible with glob patterns:
...
specific file by removing the cached response before the build. For example:
