...
- They don't just output HTML, but also the CSS and JavaScript needed on the
  client side. And the CSS and JS code is output only if the component is used
  at least once.
...
Technically, a component is an **async function** that accepts an object with
the properties and can be run in all template engines.
...
Because the `comp` variable is available inside the components, it's possible to
nest components inside other components. In JSX this is easy:
...
But this way to consume components is not very convenient. To make this easier,
