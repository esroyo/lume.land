...
Processors are functions that transform the content of pages during the build.
They are executed **after the page is rendered** so you can use the `page.text`
or `page.bytes` properties to read and modify the content of the pages (as
`string` or `Uint8Array`). Let's see an example of a processor to minify HTML
pages:
...
first argument is an array with the page extensions that will be processed. In
this example we want to process HTML pages so the array has only the `.html`
extension.
...
Note that they are executed in the same order as they are defined. This allows
chaining different processors to the same file extension. For example: two
processors for the `.css` extension, one to compile the code and other to
minify.
