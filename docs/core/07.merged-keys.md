...
When merging variables with objects, **the complete value is overridden**.
However, you may want to merge some values differently. For example, let's say
we have the following two data files, one in the root and the other in a
subfolder. Both files have a `site` variable with different values:
...
All pages in the subfolder will have the latest version of the variable that has
the `author` subkey but missing the `title` value, because the whole variable is
overridden. You can change this behaviour using the `mergeKey()` configuration
function in the `_config.ts` file:
...
This configures Lume to use the `object` mode to merge the variable `site`. Now,
the result of the variable `site` is an object including the properties of the
parent variable and only overrides the properties with the same name. So the
...
If we configure Lume to use the `array` mode for the `category` variable, the
result of this merge is:
